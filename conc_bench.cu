...
__global__ void sum_gpu_left(float *matrix, const int N)
{
...
__global__ void sum_gpu_right(float *matrix, const int N)
{
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= N / 2 && row < N)
    {
...
int main()
{
    ...
    for (int z = 0; z < RUNS; z++)
    {
        ...
        SAFE_CALL(cudaHostAlloc((void **)&d_matrix_host, N * sizeof(float),
                                cudaHostAllocMapped));
        SAFE_CALL(
            cudaHostGetDevicePointer((void **)&d_matrix, (void *)d_matrix_host, 0));
#elif UNIFIED
...
#if ZEROCOPY
            // Synchronization needed to avoid race conditions (after the CPU and GPU have
            // done their sides, we need to sync)
            CHECK_CUDA_ERROR
#endif
            // -------------------------------------------------------------------------
    ...
    if (ZEROCOPY)
        ...
        std::cout << "CP";
    std::cout << " Run time: "
              << std::accumulate(results.begin(), results.end(), 0) / float(RUNS)
              << " ms - ";
    std::cout << "CPU time only "
              << std::accumulate(cpu_results.begin(), cpu_results.end(), 0) /
                     float(RUNS)
              << " ms" << std::endl;
}
