...
class GPUExec : public rclcpp::Node
{
public:
    GPUExec() : Node("gpu_exec"), 
        current_run_(-1), 
        current_sum_id_(-1), 
        current_matrix_()
    {
        subscription_ = this->create_subscription<conc_benchmark_cuda::msg::ConcArray>(
            "random_matrix", 10, 
            std::bind(&GPUExec::retrieve_matrix_cb, this, std::placeholders::_1));
        publisher_ = this->create_publisher<conc_benchmark_cuda::msg::ConcArray>(
            "gpu_sum", 10);
        publish_timer_ = this->create_wall_timer(
            5s, std::bind(&GPUExec::publish_matrix_cb, this));

        init_gpu(&d_matrix_device_);
    }
...
private:
    void publish_matrix_cb()
    {
        ...
        sum_gpu(current_matrix_, d_matrix_device_, current_sum_id_);
    ...
    float *d_matrix_device_;
};

int main(int argc, char *argv[])
{
    rclcpp::init(argc, argv);
    rclcpp::spin(std::make_shared<GPUExec>());
    rclcpp::shutdown();
    return 0;
}